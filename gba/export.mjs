...
/* `.mjs` as well as `.js`: a browser refuses to run a module served as
   anything but JavaScript, and nooks.mjs is imported by the page. */
...
/* Where it has got to. An export prints nothing until it is finished, and one
   that had been running for thirteen hours could not say which of its stages
   it was in, or whether it was in any of them. The page reports every forty
   maps and the node side every stage, all to stderr with the seconds so far,
   so a build log shows a stall as a stage that never ended. */
const began = Date.now();
const stamp = (what) => console.error(`export: ${what} (${((Date.now() - began) / 1000).toFixed(0)}s)`);
page.on('console', (m) => { const t = m.text(); if (t.startsWith('export:')) stamp(t.slice(8)); });
...
const harvest = await page.evaluate(async ({ mapIds, ledger }) => {
  ...
  for (const id of mapIds) {
    ...
    if (mapIds.indexOf(id) % 40 === 0) console.log(`export: painting ${id}, ${mapIds.indexOf(id)}/${mapIds.length} maps`);
    const cells = [];
    const solid = [];
    const cover = [];
    const ledge = [];
...
stamp('background palette built');
